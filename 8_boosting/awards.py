...
class Transformer:
    ...
    def fit(self, X, num_bound):
        ...
        self.dict_key_words = dict(
            sorted(dict_.items(), key=lambda item: item[1])[::-1]
        )
    ...
    def transform(self, X):
        ...
        for val in {b for i in list(X["keywords"]) for b in i}:
            if (
                val in self.dict_key_words.keys()
                and self.dict_key_words[val] >= self.bound
            ):
...
def train_model_and_predict(train_file: str, test_file: str) -> ndarray:
    ...
    regressor = CatBoostRegressor(
        **{"learning_rate": 0.04626808450407065, "max_depth": 9, "n_estimators": 455},
        cat_features=catg,
        verbose=False,
